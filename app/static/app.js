...
const I18N = {
  es: {
    ...
    comparisonTitle: "Comparacion de reparacion",
    compareGeneratedLabel: "Nuevo reparado",
    compareGeneratedProvider: "Creado mezclando la transcripcion de {provider} con el original roto.",
    compareGeneratedTranscriptProvider: "Transcripcion cruda generada por {provider}; sube un SRT/VTT para repararlo.",
    compareOriginalLabel: "Original roto",
    compareOriginalHint: "SRT/VTT subido con tiempos incorrectos o drift temporal.",
    compareMatchLabel: "Badge match",
    compareMatchHint: "MATCH OK significa poco cambio; +/- ms muestra cuanto se ha movido el cue roto.",
    timeMatchColumn: "Tiempo + cambio",
    timeMatchColumnHint: "Nuevo vs roto",
    newSttColumn: "Nuevo reparado",
    newSttColumnHint: "Transcript + SRT/VTT roto",
    syncedOriginalColumn: "Original roto",
    syncedOriginalColumnHint: "SRT/VTT subido",
    providerWhisperx: "WhisperX local en la RTX",
    providerElevenlabs: "ElevenLabs API alojada",
    newSubtitles: "Nuevo reparado",
    originalSubtitles: "Original roto",
    newTimeShort: "Nuevo",
    brokenTimeShort: "Roto",
    ...
    matchSummary: "{matched} pares · {coverage}% cubierto · {delta} ms cambio medio",
    matchNoReference: "Sin original roto para comparar",
    matchNoCues: "Sin subtitulos nuevos todavia",
    matchNewOnly: "solo nuevo",
    matchOriginalOnly: "solo roto",
    matchGood: "match ok",
    ...
    wordResync: "Reparacion por palabras",
    cues: "cues",
    tokenCoverage: "cobertura tokens",
    interpolated: "interpolados",
    downloadRawSrt: "RAW STT SRT",
    downloadRawVtt: "RAW STT VTT",
    downloadRawJson: "RAW STT JSON",
    downloadRawTxt: "RAW STT TXT",
    downloadRawTsv: "RAW STT TSV",
    downloadNewSrt: "NEW SRT",
    downloadNewVtt: "NEW VTT",
    downloadNewTsv: "NEW TSV",
  ...
  en: {
    ...
    comparisonTitle: "Repair comparison",
    compareGeneratedLabel: "New repaired",
    compareGeneratedProvider: "Built by merging the {provider} transcript with the broken original.",
    compareGeneratedTranscriptProvider: "Raw transcript generated by {provider}; upload an SRT/VTT to repair it.",
    compareOriginalLabel: "Broken original",
    compareOriginalHint: "Uploaded SRT/VTT with bad timing or temporal drift.",
    compareMatchLabel: "Match badge",
    compareMatchHint: "MATCH OK means little change; +/- ms shows how far the broken cue moved.",
    timeMatchColumn: "Time + change",
    timeMatchColumnHint: "New vs broken",
    newSttColumn: "New repaired",
    newSttColumnHint: "Transcript + broken SRT/VTT",
    syncedOriginalColumn: "Broken original",
    syncedOriginalColumnHint: "Uploaded SRT/VTT",
    providerWhisperx: "local WhisperX on the RTX",
    providerElevenlabs: "hosted ElevenLabs API",
    newSubtitles: "New repaired",
    originalSubtitles: "Broken original",
    newTimeShort: "New",
    brokenTimeShort: "Broken",
    ...
    matchedSubtitles: "Match",
    matchSummary: "{matched} pairs · {coverage}% covered · {delta} ms avg shift",
    matchNoReference: "No broken original to compare",
    matchNoCues: "No new subtitles yet",
    matchNewOnly: "new only",
    matchOriginalOnly: "broken only",
    matchGood: "match ok",
...
function providerLabel(value = state.resultProvider || sttProvider.value) {
  return value === "elevenlabs" ? t("providerElevenlabs") : t("providerWhisperx");
}

function renderCompareExplainer() {
  if (!providerExplain) return;
  const key = state.compareMode === "transcript" ? "compareGeneratedTranscriptProvider" : "compareGeneratedProvider";
  providerExplain.textContent = t(key, {
...
async function loadResult() {
  ...
  const rawTranscript = result.generated_cues || [];
  const uploadedReference = result.uploaded_reference_cues || [];
  const repairedReference = result.reference_cues || [];
  const repairAvailable = Boolean(result.reference_resync?.available && uploadedReference.length && repairedReference.length);
  state.rawTranscript = rawTranscript;
  state.generated = repairAvailable ? repairedReference : rawTranscript;
  state.words = result.word_timestamps || [];
  state.referenceRaw = uploadedReference.length ? uploadedReference : [];
  state.downloads = result.downloads || {};
  state.alignment = result.alignment || {};
  state.resultProvider = result.stt_provider || sttProvider.value;
  state.compareMode = repairAvailable ? "repair" : "transcript";
  state.waveform = {
    status: "idle",
    peaks: [],
    duration: 0,
  ...
  state.merged = mergeCues(state.generated, state.reference, { pairByIndex: state.compareMode === "repair" });
...
function downloadLabel(kind) {
  const labels = {
    srt: "downloadRawSrt",
    vtt: "downloadRawVtt",
    json: "downloadRawJson",
    txt: "downloadRawTxt",
    tsv: "downloadRawTsv",
    sync_srt: "downloadNewSrt",
    sync_vtt: "downloadNewVtt",
    sync_tsv: "downloadNewTsv",
  };
  return labels[kind] ? t(labels[kind]) : kind.toUpperCase();
}
...
function renderDownloads(downloads, alignment) {
  ...
  Object.entries(downloads).forEach(([kind, url]) => {
    ...
    link.href = url;
    link.textContent = downloadLabel(kind);
    link.download = "";
...
function mergeCues(generated, reference, options = {}) {
  if (options.pairByIndex) {
    const referenceByIndex = new Map(reference.map((cue) => [Number(cue.index), cue]));
    const pairedReferenceIndexes = new Set();
    const rows = generated.map((cue) => {
      const referenceCue = referenceByIndex.get(Number(cue.index)) || null;
      if (referenceCue) pairedReferenceIndexes.add(Number(referenceCue.index));
      return {
        start: cue.start,
        end: cue.end,
        generated: cue,
    ...
    reference.forEach((cue) => {
      if (pairedReferenceIndexes.has(Number(cue.index))) return;
      rows.push({ start: cue.start, end: cue.end, generated: null, reference: cue });
    });
...
function renderCueGrid() {
  ...
  state.merged.forEach((row, index) => {
    ...
    const timeContent =
      state.compareMode === "repair" && row.reference
        ? `
          <span class="time-main">${escapeHtml(t("newTimeShort"))} ${formatTime(row.generated?.start ?? row.start)}</span>
          <span class="time-aux">${escapeHtml(t("brokenTimeShort"))} ${formatTime(row.reference.start)}</span>
        `
        : `<span class="time-main">${formatTime(row.start)}</span>`;
    const item = document.createElement("button");
    item.type = "button";
    item.className = `cue-row match-${match.kind}`;
    item.dataset.index = String(index);
    item.innerHTML = `
      <div class="time">${timeContent}<span class="row-match ${match.kind}">${escapeHtml(match.label)}</span></div>
...
function updateProviderControls() {
  ...
  if (!state.resultProvider) renderCompareExplainer();
}
